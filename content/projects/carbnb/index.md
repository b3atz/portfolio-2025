+++

title = "CarBNB"

date = 2023-01-17

[extra]

timeRange = "01/17/2023 - 06/05/2023"

image = "portfolio.jpg"

desc="CarBNB is software that would allow for users to temporally rent privately owned car chargers. This network would connect charger owners and electric car owners"

repoLink="https://github.com/b3atz/portfolio-2025"

demoLink="https://www.youtube.co/watch?v=xOoj4kk-Qec"

+++
# Carbnb

Technologies and skills summary: React, git, Postman, Google Auth, Firebase, google cloud, Bash, Docker, Teamwork, Communication, Public Speaking, Agile

## Summary

This project was completed during capstone, a two quarter long project course that pairs student teams and project sponsors. The process involved planning and programming phases and agile like sprints. This project provided good real word experience like self evaluations, meetings and presentations with project sponsors, and collaboration with different teams (front-end and back-end teams).

## Contributions

My roll and contributions including working on the fronted, building scripting software to deploy our site to google cloud, and setting the API document in postman to create allow our front-end and back-end to communicate. One example of my work that i took pride in was calling a meeting to do a quick rundown of collaborating with git. Often times students at PSU have used git, but have not worked with collaborators in git, so i prepared some demonstrations of how to re-base and squash commits. I the team found this useful, and i believed it was a good use of time to allow everyone in our group the ability to contribute.

Overall this project was a great learning experience and helped me grow as a software Engineer. Below are our project presentation and my performance reviews

## Supporting Documents

### Presentation

[https://www.youtube.co/watch?v=xOoj4kk-Qec](hps://www.youtube.com/watch?v=xOoj4kk-Qec)

### Reviews

```text
===================================================

Portland State University

Maseeh College of Engineering and Computer Science

Computer Science Capstone Program

Spring-Summer 2023

Final 360 Review Report

===================================================

  

Software Engineer: Collin Werth

  

Self Assessment

---------------

TECHNICAL     : 5

WORKETHIC     : 3

COMMUNICATION : 4

COOPERATION   : 3

INITIATIVE    : 3

TEAM          : 4

CONTRIBUTION  : 4

Strengths        -> "Worked well when it counted"

Weaknesses       -> "Was a little absent from contributing to project"

  

Ratings from Teammates

-------------------------------

TECHNICAL     : ratings=[5, 4, 4, 3]  avg=4.00

WORKETHIC     : ratings=[4, 4, 3, 3]  avg=3.50

COMMUNICATION : ratings=[4, 4, 3, 2]  avg=3.25

COOPERATION   : ratings=[5, 5, 4, 4]  avg=4.50

INITIATIVE    : ratings=[5, 4, 4, 3]  avg=4.00

TEAM          : ratings=[5, 5, 4, 3]  avg=4.25

CONTRIBUTION  : ratings=[5, 4, 4, 4]  avg=4.25

  

Strengths

-----------------

"Great front-end engineer. Couldn't have been more appropriate for the role."

  

"Contributed to the project overall and not just what he was responsible for. Tried his best to help others when anyone was struggling."

  

"Was great at setting up and working on testing. Has a good grasp of a range of topics."

  

"Very good with front end development. Very good with seeking ways to make the project easier for team members. Provide good insight into project for members who have less experience with software development. "

  
  

Weaknesses

-----------------

"Communication during in-person meetings were great, but messages in the team Discord and DMs often went unanswered."

  

"Writing cleaner and more readable code, better formatting."

  

"Can't think of any in particular"

  

"None that I can think of, a solid software developer. "

  
  

Trait to Improve

-----------------

"Being more detail oriented."

  

"Focus on staying up to date with pulling often and emulating all parts of the project that are required to test your parts."

  

"None that I can think of, pleasant to work with."

  
  

Averages Across All Engineers on All Teams

------------------------------------------

TECHNICAL    : 4.44 (stddev = 0.65)

WORKETHIC    : 4.57 (stddev = 0.68)

COMMUNICATION: 4.28 (stddev = 0.84)

COOPERATION  : 4.42 (stddev = 0.82)

INITIATIVE   : 4.39 (stddev = 0.85)

TEAM         : 4.48 (stddev = 0.73)

CONTRIBUTION : 4.47 (stddev = 0.70)
```
